...
public class BoardController extends HttpServlet {
	...
	private BoardService bsv; //이직 미구현 : 앞으로 생성할 예정 (service폴더 interface 생성 -> 저장)
	...
	protected void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		case "insert":
			//insert -> 글쓰기 등록 버튼을 눌렀을 때 글을 등록 -> list 페이지로 이동
		...
		case "update":
			...
			break;
		case "remove":
			//remove -> 삭제가 이루어지고 번호를 이용하여 게시물 삭제 -> list로 이동
			try {
				//번호에 해당하는 게시물을 삭제
				int bno = Integer.parseInt(req.getParameter("bno"));
				isUp = bsv.remove(bno);
